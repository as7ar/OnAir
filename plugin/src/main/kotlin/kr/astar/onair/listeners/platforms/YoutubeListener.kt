...
class YoutubeListener : YouTubeEventListener {
    ...
    private fun getPlayerByVideoId(videoId: String): Player? {
        ...
        if (uuid == null) {
            plugin.log.warning(translate("system.boom", mapOf("err" to "uuid parse error ($uuidStr)")))
            return null
        }

        val player = Bukkit.getPlayer(uuid)
        if (player == null) {
            plugin.log.warning(translate("system.boom", mapOf("err" to "player not found or offline ($uuid)")))
            return null
        }
