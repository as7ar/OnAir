...
import kr.astar.onair.events.StreamingDonateEvent
...
class EventManager(private val platforms: Platforms) {
            ...
            asynchronously {
                ...
                runTask {
                    when (userData.getMessageTarget()) {
                        ...
                        else -> Bukkit.broadcastMessage(formatted.toLegacyString())
                    }
                }
            }
        ...
        safely(player) {
            asynchronously {
                ...
                if (ConfigSet.donation.showTitle) {
                    player.sendTitle("", titleComponent.toLegacyString())
                }
    ...
    private fun executeCommand(player: Player, donate: DonateContent) {
        ...
        else player.performCommandAsOP(parsed)
    }

    private fun callChatEvent(player: Player, chat: ChatContent) {
        runTask {
            Bukkit.getPluginManager().callEvent(StreamingChatEvent(
    ...
    private fun callDonateEvent(player: Player, donate: DonateContent) {
        runTask {
            Bukkit.getPluginManager().callEvent(StreamingDonateEvent(
