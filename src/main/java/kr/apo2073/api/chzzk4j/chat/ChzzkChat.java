...
import kr.apo2073.api.chzzk4j.chat.event.ChzzkEvent;
...
public class ChzzkChat {
    ...
    public void emit(Class<? extends ChzzkEvent> clazz, ChzzkEvent obj) {
        if (handlerMap.containsKey(clazz)) {
            for (Consumer<? extends ChzzkEvent> handler : handlerMap.get(clazz)) {
                @SuppressWarnings("unchecked")
                Consumer<ChzzkEvent> specificHandler = (Consumer<ChzzkEvent>) handler;
