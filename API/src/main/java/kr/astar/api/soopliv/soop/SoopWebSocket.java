package kr.astar.api.soopliv.soop;

import kr.astar.api.soopliv.SSLUtils;
import kr.astar.api.soopliv.data.Chat;
import kr.astar.api.soopliv.data.Donate;
import kr.astar.api.soopliv.data.User;
import kr.astar.api.soopliv.soop.listener.SoopEventListener;
import kr.astar.api.soopliv.utilities.Debugger;
...
public class SoopWebSocket extends WebSocketClient {
    ...
    public Map<String, String> getSoopUser() {
        return soopUser;
    }
    ...
    public void onMessage(ByteBuffer bytes) {
        ...
        if (CONNECT_RES_PACKET.equals(message)) {
            ...
            send(JOIN_PACKET.getBytes(StandardCharsets.UTF_8));
            return;
        }
    ...
    public void kill() {
        try {
            ...
            try { stopConnectionLostTimer(); } catch (Exception ignored) {}
            try { if (getConnection() != null) getConnection().closeConnection(1000, "Killed"); } catch (Exception ignored) {}
            try { if (getSocket() != null && !getSocket().isClosed()) { getSocket().shutdownInput(); getSocket().shutdownOutput(); getSocket().close(); } } catch (Exception ignored) {}

            try {
                Thread t = new Thread(() -> {
                    try { super.closeBlocking(); } catch (Exception ignored2) {}
                });
                t.setDaemon(true);
                t.start();
                t.join(1000);
